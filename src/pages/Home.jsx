    ...
    return (
        <section id="home" className="grid md:grid-cols-2 grid-cols-1 items-center min-h-screen md:px-20 px-6 pt-24 md:gap-10 gap-0 overflow-hidden">
            <div className="flex flex-col  md:mt-0 mt-6">
                <h5 className="text-(--tertiary) font-bold mb-2 md:mb-4 text-xs md:text-base tracking-widest uppercase font-space">{homeData.subHeading}</h5>
                <h1 className="text-white font-black text-2xl md:text-5xl leading-tight mb-2 md:mb-4">{homeData.heading[0]}<span className="text-(--primary) block">{homeData.heading[1]}</span>{homeData.heading[2]}</h1>
                <p className="text-gray-400 md:text-lg text-sm leading-relaxed mb-8 w-3/4 md:w-2/3 text-justify">{homeData.description}</p>
